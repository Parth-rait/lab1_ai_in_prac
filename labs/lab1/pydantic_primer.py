...
class TicketRecord(BaseModel):
    ...
    category: CATEGORIES = Field(
        description="billing = money in. claims = an actual or intended claim. policy_change = altering the contract. technical = the app or portal is broken. complaint = Aurora's conduct is the subject. information = a question with no pending transaction."
    )
    urgency: int = Field(
        ge=1, 
        le=5,
        description="Urgency 1-5. 5 = emergency in progress or a same-day deadline. 1 = a general question, nothing at stake."
    )
    policy_number: str | None = Field(
        default=None, 
        pattern=r"^AUR-\d{7}$",
        description="Format AUR- followed by exactly 7 digits, copied character for character. null if the ticket contains no such string. Never invent or reformat one."
    )
