...
# --- Lab 1 Part C: business rules -------------------------------------------
def test_apply_business_rules_escalate_on_high_urgency():
    out = apply_business_rules({"urgency": 4}, "everything is fine, thanks")
    assert out["escalate"] is True


def test_apply_business_rules_escalate_on_ombudsman_mention():
    out = apply_business_rules({"urgency": 1}, "I am filing a complaint with the Ombudsman")
    assert out["escalate"] is True


def test_apply_business_rules_no_escalate_otherwise():
    out = apply_business_rules({"urgency": 3}, "please add my newborn to the policy")
    assert out["escalate"] is False


def test_extract_deterministic_ignores_policy_number_in_quoted_reply():
    ticket = (
        "What documents do I need for reimbursement?\n\n"
        "> On 1 Jan 2026, Aurora Support wrote:\n"
        "> Your reference number is AUR-1234567.\n"
    )
    assert extract_deterministic(ticket)["policy_number"] is None


def test_extract_deterministic_finds_policy_number_in_live_body():
    ticket = "My policy AUR-7654321 has not renewed."
    assert extract_deterministic(ticket)["policy_number"] == "AUR-7654321"


def test_extract_deterministic_excludes_auroras_own_email_from_pii():
    ticket = (
        "Where is my e-card?\n\n"
        "> On 1 Jan 2026, Aurora Support <support@aurorahealth.example> wrote:\n"
        "> Thank you for contacting Aurora.\n"
    )
    assert extract_deterministic(ticket)["contains_pii"] is False


def test_extract_deterministic_flags_a_real_phone_number_as_pii():
    ticket = "Please call me back on 9876543210."
